...
import { prepareAdapter, startAdapter, stopAdapter } from "./adapters/index.js";
import { bindAdapter, ingest } from "./ingest/pipeline.js";
import { startScheduler, stopScheduler } from "./scheduler.js";
import { groupsRouter, syncAllGroupsFromAdapter, syncGroupsFromAdapter } from "./routes/groups.js";
...
const defaultModel = "balanced-model";
  ...
  try {
    ...
    res.json({ 
      models: cachedModels.length > 0 ? cachedModels : [
        { modelId: "default-model", name: "CodeBuddy 默认模型" }
      ],
      defaultModel 
    });
  } catch (error: any) {
    ...
    res.json({
      models: [
        { modelId: "default-model", name: "CodeBuddy 默认模型" },
        { modelId: "balanced-model", name: "CodeBuddy Balanced" }
      ],
      defaultModel,
  ...
  try {
    ...
    for await (const msg of stream) {
      ...
      if (msg.type === "system" && (msg as any).subtype === "init") {
        ...
        // SDK 将工具结果作为 user 消息中的 tool_result 内容块返回。
...
/**
 * 启动群聊接入：准备平台适配器 → 把消息接入管道 → 启动适配器与定时调度
 */
async function bootstrap(): Promise<void> {
  const savedAdapter = db.getSetting<unknown>('adapter');
  if (savedAdapter === 'onebot11') {
    config.adapter = savedAdapter;
  }

  // 设置页保存的 OneBot 参数在重启后覆盖环境变量默认值；未保存的字段仍沿用 .env。
  const savedOnebot = db.getSetting<Partial<typeof config.onebot>>('onebot');
  if (savedOnebot) {
    Object.assign(config.onebot, savedOnebot);
  }

  try {
    const adapter = await prepareAdapter();
    db.markAllPlatformAccountsOffline();
    bindAdapter(adapter);
    adapter.onMessage((msg) => {
      void ingest(msg).catch((err) => console.error("[ingest] 处理消息失败:", err));
    });
    adapter.onStatusChange?.((status, info) => {
      console.log(`[adapter] 状态变化: ${status}${info ? ` - ${info}` : ""}`);
      eventBus.emitEvent({ type: "status", adapter: adapter.platform, status, info });
    });
    adapter.onAccountStatusChange?.((account, info) => {
      db.upsertPlatformAccount({
      ...
      eventBus.emitEvent({
        ...
        status: account.status,
        info: `${account.selfId}${info ? ` - ${info}` : ''}`,
      });
      if (account.status === 'connected') {
        void syncGroupsFromAdapter(adapter, account.selfId).catch((err) =>
          console.error(`[groups] 账号 ${account.selfId} 自动同步群列表失败:`, err)
        );
      }
    });

    await startAdapter();
    if (adapter.getAccounts().some((account) => account.status === 'connected')) {
      await syncAllGroupsFromAdapter(adapter);
    }
  } catch (err) {
    console.error(`[bootstrap] 适配器「${config.adapter}」启动失败:`, err);
  }

  try {
    startScheduler();
  } catch (err) {
    console.error("[bootstrap] 调度器启动失败:", err);
  }
}
...
async function shutdown(signal: string): Promise<void> {
  ...
  try {
    ...
    console.error("[shutdown] 清理失败:", err);
  }
  process.exit(0);
}
